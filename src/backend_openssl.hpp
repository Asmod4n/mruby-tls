// The OpenSSL backend, entire. Included by exactly one translation unit
// (src/mrb_tls.cpp) and never compiled on its own, which is why it is a
// header: only one backend is ever built, so there is nothing to link
// against and no internal interface worth designing. Each backend
// defines the whole surface - the Ruby classes, gem_init, and the
// MRB_API functions from include/mruby/tls.h - so the two never have to
// agree on anything except that header.
#ifndef MRUBY_TLS_BACKEND_OPENSSL_HPP
#define MRUBY_TLS_BACKEND_OPENSSL_HPP
...
#endif  // MRUBY_TLS_BACKEND_OPENSSL_HPP
